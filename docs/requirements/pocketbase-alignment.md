...
- `candidates` (base): candidate profile + documents (file fields) + `status`
...
- `_pb_users_auth_` (auth): `role` select (`administrator|manager|recruiter|staff`)
...
- Candidate Records
  ...
  - Implemented (documents): `resume` (pdf), `passport` (file), `visa` (file), `others` (multi-file)
...
- Document Tracking
  - Implemented (partial): candidate docs exist as fixed file fields + “others”; employer contracts exist as file field
  - Implemented: `documents` collection for typed documents, status, expiry, and verification metadata
...
- User Roles & Access: partially implemented (`role` exists but values don’t exactly match the checklist’s naming)
  - Implemented: role values include `recruiter` (legacy: `staff`)
...
- Consider migrating fixed “candidate document” fields (`resume`, `passport`, `visa`, `others`) into `documents` over time for a single source of truth
- Decide final role names (`staff` vs `recruiter`) and remove legacy values once all users are migrated
