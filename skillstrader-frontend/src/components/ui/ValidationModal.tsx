...
export default function ValidationModal({
  ...
  return (
    ...
    >
      ...
      {/* Modal card */}
      <div className="relative z-10 w-full max-w-sm bg-white rounded-2xl shadow-2xl border border-[var(--border)] overflow-hidden animate-[fadeInScale_0.18s_ease]">
        {/* Header */}
        <div className="px-6 pt-6 pb-4">
          <div className="flex items-start gap-3">
            {/* Icon */}
            <div className={`mt-0.5 p-2 shrink-0 w-9 h-9 rounded-xl flex items-center justify-center ${destructive ? 'bg-[#fff0ed]' : 'bg-[var(--surface2)]'}`}>
              {destructive ? (
            ...
            <div>
              <h2 id="vm-title" className="text-lg font-bold text-[var(--text)] leading-tight m-0">
                {title}
              </h2>
              <p id="vm-message" className="mt-1.5 text-base text-[var(--muted)] leading-relaxed m-0">
                {message}
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-end gap-2.5 px-6 pb-5">
          <button
            ref={cancelRef}
            type="button"
            className="px-4 py-2 rounded-full border border-[var(--border)] bg-white text-[var(--text)] text-base font-semibold cursor-pointer hover:bg-[var(--surface2)] transition-colors"
            onClick={onCancel}
          >
            {cancelLabel}
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-full text-base font-semibold cursor-pointer border-none transition-all hover:brightness-110 active:scale-95 ${
              destructive
                ? 'bg-gradient-to-br from-[var(--primary)] to-[var(--primary2)] text-white shadow-[0_4px_14px_rgba(200,75,49,0.25)]'
